...
本仓库的专有许可仅覆盖 YuanX1a0 拥有的代码、文档与原创模型。第三方组件由使用者分别取得，仍适用各自的许可；仓库可见性不会改变第三方条款。
...
原始代码版权归属由仓库所有者确认。本版本采用专有许可；先前按 GPL 发布的版本仍保留已经授予接收者的权利。本次不删除或改写旧公开仓库的许可证与历史。参阅 [GNU 对版权持有者另行许可的说明](https://www.gnu.org/licenses/gpl-faq.en.html#CanDeveloperThirdParty)。
