...
FiveM 警灯与警笛控制器JavaScript 和 C# OneSync 。，项目仍采用专有许可，许可见 [LICENSE](LICENSE)。

仓库不包含任何第三方警笛录音、音频、RageUI 源码或游戏动画文件。默认可使用 GTA 原生警笛；警笛包需使用者从对应作者自行下载并安装。下载链接和完整步骤见 [警笛安装说明](docs/audio-installation.md)。
...
2. 将本仓库作为 `yx_sirencontrol` 放入服务器 `resources`，目录名必须完全一致。
...
资源目录名必须严格为 **`yx_sirencontrol`**，区分大小写。`YX_SIRENCONTROL`、`yx_sirencontrol-main` 或其他改名都不能使用
...
非紧急汽车先 `/siren on`，主驾驶输入 `/putsiren`。

I 菜单的“便携 LED 位置”可调左右、前后、高低：每次 1 cm，各轴 ±200 cm，实时应用并自动保存。
